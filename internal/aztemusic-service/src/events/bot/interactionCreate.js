    ...
    async execute(interaction, client) {
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.values[0] == "melody_help_category_general") {
                embed.setAuthor({ name: `${config.appName} Help` });
                embed.setTitle("General Commands");
                embed.setDescription(`**/help** - Shows all ${config.appName} commands available.\n**/stats** - View ${config.appName} statistics.\n**/botinfo** - View more information about ${config.appName}.`);
